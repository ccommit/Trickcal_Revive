...
`Infra.Editor`는 지금 빈 자리다 — 에디터 전용 도구(마스터데이터 생성기 등, Before의 `CharacterMasterExporter.cs`에 해당)가 들어갈 asmdef이며, 필요해지는 시점에 해당 컨텐츠 문서(예: `06_사도성장_설계서.md`)가 채운다. 지금 막힌 것은 없다(결정 2026-08-05).
...
│                 IAccountRepository, ICurrencyRepository, IPlayerCharacterRepository,
│                 IPartyRepository, IStageProgressRepository, IInventoryRepository,
│                 IGrowthRepository, IAccountAuthRepository
   ├ LoginSceneInstaller   → (씬 전용 등록 없음, 루트 서비스만 사용)
   ├ MainSceneInstaller    → PartyFormationValidator, CommandHistory(팩토리)
   └ BattleSceneInstaller  → 전투 서비스 8종(§4, `04_전투_설계서.md` 참고) + BattlePhase 전이 규칙
```

**수정(결정 2026-08-05, 개발 구현 중 발견).** 최초안은 `PlayerDataRepository`/`AccountAuthRepository` 기반 인터페이스 8종을 `MainSceneInstaller`/`LoginSceneInstaller`에 등록했다. 그런데 §2.1이 이미 "씬 컨테이너는 **루트만** 부모로 삼는다(씬↔씬 체이닝 아님)"고 정해뒀다는 걸 놓쳤다 — 그 결과:

- `07_인벤토리재화_설계서.md`의 `RewardService`(**Battle씬**의 `ResultOverlayController`가 호출)가 `IInventoryRepository`/`ICurrencyWallet`을 못 씀(Main씬에만 등록돼 있었음)
- `01_계정인증_설계서.md`의 `SettingsController`(**Main씬**)가 `IAccountAuthRepository.DeleteAccount`를 못 씀(Login씬에만 등록돼 있었음)

두 경우 다 "이 서비스가 필요한 화면"과 "이 서비스가 등록된 씬"이 어긋난 것이 원인이다. `PlayerDataRepository`/`AccountAuthRepository`는 씬 특유의 상태가 없는 **무상태 데이터 접근자**(`PlayerDataRepository`는 호출마다 파일을 새로 읽는다, `구조개선_적용계획.md` §17)라 `MasterDataRepository`와 마찬가지로 루트에 두는 것이 맞다 — Before 실제 코드가 원래 이렇게 했던 이유이기도 하다. 반대로 전투 서비스는 **"이전 판 상태가 다음 판으로 새면 안 된다"**는 이유로 씬 컨테이너에 남아야 한다(씬을 나가면 컨테이너와 함께 사라져야 함) — 이건 그대로 유지한다. `PartyFormationValidator`/`CommandHistory`도 Main씬 진짜 전용 상태(파티 편성 세션)라 그대로 둔다.

`LoginSceneInstaller`는 이제 씬 전용 등록이 없다 — `InstallBindings`는 비워두고, `InjectSceneObjects`가 루트에서 `IAccountAuthRepository`/`ISessionService`를 `Resolve`해 `AuthController.Configure(...)`에 넘기기만 한다.
