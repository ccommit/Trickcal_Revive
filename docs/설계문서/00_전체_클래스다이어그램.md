...
- 확정 완료(2026-08-05, 개발 구현 중 발견·수정): `09` §2.2 DI 등록 위치 오류 — `PlayerDataRepository`/`AccountAuthRepository` 기반 인터페이스 8종을 씬(Main/Login) 대신 **루트 컨테이너**로 이동. 씬 컨테이너가 루트만 부모로 삼는 규칙(§2.1) 때문에 Battle씬의 `RewardService`, Main씬의 `SettingsController.DeleteAccount`가 각각 다른 씬에 등록된 인터페이스를 못 쓰던 문제 해소. `09` §5.2 "10개 확정" 오타도 11개로 정정. `Infra.Editor` asmdef는 빈 자리로 유지(필요해지면 해당 컨텐츠 문서가 채움).
