#!/usr/bin/env python3
"""Recover the issue8 lobby chrome from the read-only reference snapshot.

The existing issue2 lobby manifest is intentionally left untouched.  This script
creates a separate, deterministic set for the corrected profile, currency chrome,
and the three visible lobby buttons.
"""
...
try:
    ...
    raise SystemExit("Pillow is required. Use the bundled Codex Python runtime.") from exception
...
OUTPUT_SPRITES_RELATIVE = Path("Assets/Game/Content/UI/Lobby/Chrome")
OUTPUT_FONT_RELATIVE = Path("Assets/Game/Content/UI/Fonts/ONE Mobile POP.ttf")
OUTPUT_MANIFEST_RELATIVE = Path("Recovery/Manifests/lobby-ui-chrome.json")
...
ATLAS_SPECS = (
    AtlasSpriteSpec(
        "MainLobby_BtnBg",
        "02.resources/01.mainlobby/sprites/mainlobby/MainLobby_BtnBg.json",
        "bottom-navigation-base",
    ),
    AtlasSpriteSpec(
        "GachaButton",
        "02.resources/01.mainlobby/sprites/mainlobby/GachaButton.json",
        "bottom-navigation-recruit",
    ),
    AtlasSpriteSpec(
        "HeroButton",
        "02.resources/01.mainlobby/sprites/mainlobby/HeroButton.json",
        "bottom-navigation-apostle",
    ),
    AtlasSpriteSpec(
        "MainLobby_UserInfoBase",
        "02.resources/01.mainlobby/sprites/mainlobby/MainLobby_UserInfoBase.json",
        "player-info-base",
    ),
    AtlasSpriteSpec(
        "MainLobby_LevelBase",
        "02.resources/01.mainlobby/sprites/mainlobby/MainLobby_LevelBase.json",
        "player-level-base",
    ),
    AtlasSpriteSpec(
        "TopMenu_Base",
        "02.resources/99.common/sprites/commonicons/TopMenu_Base.json",
        "top-currency-value-base",
    ),
    AtlasSpriteSpec(
        "TopMenu_CurrencyBase",
        "02.resources/99.common/sprites/commonicons/TopMenu_CurrencyBase.json",
        "top-currency-plus-base",
    ),
)
...
def resolve_source_root(project_root: Path, explicit_root: Path | None) -> Path:
    ...
    for candidate in candidates:
        ...
        if (resolved / "00_original_apk/trickcal.apk").is_file() and (resolved / "file_manifest.csv").is_file():
            return resolved
    raise RuntimeError(
        "Recovery source was not found. Pass --source-root or configure TRICKCAL_REFERENCE_ROOT."
    )
...
def validate_source_identity(project_root: Path, source_root: Path) -> None:
    ...
    for expected in identity["files"]:
        ...
        if not source_path.is_file():
            ...
            continue
        if source_path.stat().st_size != expected["bytes"] or sha256(source_path) != expected["sha256"]:
...
def find_exactly_one(root: Path, pattern: str, description: str) -> Path:
    matches = sorted(root.glob(pattern))
    if len(matches) != 1:
        raise RuntimeError(f"Expected exactly one {description} matching {pattern}, found {len(matches)}")
    return matches[0]
...
def find_atlas_page(raw_root: Path, tag: str) -> Path:
    # The separators are deliberate.  A broad '*MainLobby*' match also selects
    # character atlases such as AmeliaR41MainLobby and silently produces junk.
    candidates = [
        path
    ...
    if len(candidates) != 1:
        raise RuntimeError(f"Expected exactly one canonical atlas page for {tag}, found {len(candidates)}")
    return candidates[0]


def make_texture_meta(guid: str, pivot: dict[str, Any], pixels_per_unit: float, border: dict[str, Any] | None = None) -> bytes:
...
def make_font_meta(guid: str) -> bytes:
...
def deterministic_guid(project_root: Path, output_path: Path) -> str:
    seed = f"trickcal-recovery-lobby-ui-chrome:{output_path.relative_to(project_root).as_posix()}"
    return hashlib.md5(seed.encode("utf-8")).hexdigest()


def prepare_atlas_sprites(project_root: Path, raw_root: Path) -> list[PreparedAsset]:
    atlas_cache: dict[str, tuple[Path, dict[str, dict[str, Any]], Path, Image.Image]] = {}
    ...
    for spec in ATLAS_SPECS:
        ...
        if atlas_tag not in atlas_cache:
            ...
            page_path = find_atlas_page(raw_root, atlas_tag)
            atlas_cache[atlas_tag] = atlas_path, render_map, page_path, Image.open(page_path).convert("RGBA")

        atlas_path, render_map, page_path, page = atlas_cache[atlas_tag]
        ...
        if x < 0 or y < 0 or x + width > page.width or y + height > page.height:
            raise RuntimeError(f"Sprite rect is outside canonical atlas page for {sprite_path}")
        crop = page.crop((x, page.height - y - height, x + width, page.height - y))
        if crop.getbbox() is None:
            raise RuntimeError(f"Recovered crop is empty for {sprite_path}")
        ...
        prepared.append(
            PreparedAsset(
                category="lobby-ui-chrome",
                purpose=spec.purpose,
                evidence_status="confirmed",
                primary_source=page_path,
                ...
                transformation="crop-canonical-spriteatlas-page",
                parameters={
                    "atlasTag": atlas_tag,
                    "atlasPageFileName": page_path.name,
                ...
                evidence="Serialized Sprite key matched the atlas render map and the exact canonical atlas page name.",
            )
        )
...
def prepare_adventure(project_root: Path, raw_root: Path) -> PreparedAsset:
    source = raw_root / "02.resources/addressableresources/rawsprites/mainlobby/MainLobby_BattleBtn_Uros.png"
    serialization = source.with_suffix(".json")
    if not source.is_file() or not serialization.is_file():
        raise FileNotFoundError(source)
    sprite = read_json(serialization)
    output_path = project_root / OUTPUT_SPRITES_RELATIVE / source.name
    return PreparedAsset(
        category="lobby-ui-chrome",
        purpose="bottom-navigation-adventure",
        evidence_status="confirmed",
        primary_source=source,
        supporting_sources=[("sprite-serialization", serialization)],
        output_path=output_path,
        ...
        parameters={
            "pivot": sprite["m_Pivot"],
            "pixelsPerUnit": sprite["m_PixelsToUnits"],
            "selectionEvidence": "Visual match to the supplied lobby reference screenshot.",
        },
        evidence="The raw Uros sprite matches the visible adventure-button character in the supplied screenshot.",
    )


def prepare_font(project_root: Path, raw_root: Path) -> PreparedAsset:
    source = raw_root / "02.resources/addressableresources/fonts/ONE Mobile POP.ttf"
    if not source.is_file():
        raise FileNotFoundError(source)
    output_path = project_root / OUTPUT_FONT_RELATIVE
    return PreparedAsset(
        category="lobby-ui-font",
        purpose="korean-lobby-labels",
        evidence_status="confirmed",
        primary_source=source,
        supporting_sources=[],
        output_path=output_path,
        output_bytes=source.read_bytes(),
        meta_bytes=make_font_meta(deterministic_guid(project_root, output_path)),
        transformation="byte-for-byte-copy",
        parameters={"fontName": "ONE Mobile POP", "licenseMetadata": "Reference importer reports Free"},
        evidence="The font file is the reference project's Korean UI font. Distribution rights were not independently verified.",
    )
        ...
        entries.append(
            {
                **common,
                ...
                "notes": "Unity runtime UI asset for the issue8 lobby screen.",
            }
        )
        entries.append(
            {
...
def main() -> None:
    ...
    prepared = prepare_atlas_sprites(project_root, raw_root)
    prepared.append(prepare_adventure(project_root, raw_root))
    prepared.append(prepare_font(project_root, raw_root))
    if len(prepared) != 9:
        raise RuntimeError(f"Expected exactly 9 issue8 lobby chrome resources, got {len(prepared)}")
    validate_conflicts(prepared)

    if args.validate_only:
        print(f"Validated source identity and planned {len(prepared)} lobby chrome resources; no files were written.")
        return
    ...
    print(f"Recovered {len(prepared)} issue8 lobby chrome resources: {manifest_path}")
